    ...
    for(int n = 1; n <= argc; n ++) {
        ...
        if(n == argc) {
            ...
            if(strcmp("--", arg) == 0) {
                ...
                fc++;
                f = stdin;
            } else if(is_opt && strncmp("--", arg, 2) == 0) {
                if(strcmp("--block", arg) == 0) {
                    cmode = 'b';
                    FILLA(cflag, 10, 0);
                }
                else if(strncmp("--channel=", arg, 10) == 0) {
                    if(strlen(arg) == 11) {
                        arg += 10;
                        if(*arg == 'l' || *arg == 'L') cflag[0] = 1;
                        else if(*arg == 'r' || *arg == 'R') cflag[1] = 1;
                        else {
                            char c = *arg - '0';
                ...
                else if(strcmp("--no-null", arg) == 0) {
                    smode = 'n';
                }
                else if(strcmp("--left-no-null", arg) == 0) {
                    cmode = 'c';
                    FILLA(cflag, 10, 0);
                    cflag[0] = 1;
                    smode = 'n';
                }
                else if(strcmp("--report", arg) == 0) cmode = 'r';
                ...
                continue;
            } else if(is_opt && strncmp("-", arg, 1) == 0) {
                if(strcmp("-b", arg) == 0) {
                    cmode = 'b';
                    FILLA(cflag, 10, 0);
                }
                ...
                else if(strcmp("-r", arg) == 0) cmode = 'r';
                else {
                    fprintf(stderr, "*Unknown option: %s\n", arg);
                    return 1;
                }
        ...
        if(cmode == 'r') printf("RIFF WAV header checked and audio data size is %u bytes\n", data_size);
        ...
        if(cmode == 'b') {
            ...
            if(smode == 'a') {
                ...
                for(unsigned int c = 0; c < nch; c ++) {
                    ...
                    if(nch < 10 && cflag[c]) printf("%08X %01u* %s\n", crcc[c], c, arg);
                }
            } else {
                if(data_size > 0) {
                    if(data_size - ts < rs) rs = data_size - ts;
